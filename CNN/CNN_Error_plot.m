...
figure; 
subplot(1,3,1);
plot(eps_macro_prediction(:,1),eps_macro_validation(:,1),'r*');
hold on
...
legend([char(949),'_{xx,CNN}^{M} vs.',char(949),'_{xx, SCA}^{M}'],...
       [char(949),'_{xx,SCA}^{M} vs.',char(949),'_{xx, SCA}^{M}'], 'Location', 'southeast')
set(gca,'FontSize',fs);
xlabel([char(949),'_{xx,CNN}^{M}'], 'fontsize', 16);
ylabel([char(949),'_{xx,SCA}^{M}'], 'fontsize', 16);
xlim([0 0.05])
ylim([0 0.05])
ax = gca;
ax.FontSize = 16;
...
subplot(1,3,2);
plot(eps_macro_prediction(:,2),eps_macro_validation(:,2), 'blue*');
hold on
...
T = text(max(get(gca, 'xlim'))*0.2, max(get(gca, 'ylim'))*0.8, str); 
set(T, 'fontsize', 16, 'verticalalignment', 'top', 'horizontalalignment', 'left');
legend([char(949),'_{yy,CNN}^{M} vs.',char(949),'_{yy, SCA}^{M}'],...
       [char(949),'_{yy,SCA}^{M} vs.',char(949),'_{yy, SCA}^{M}'], 'Location', 'southeast')
set(gca,'FontSize',fs);
xlabel([char(949),'_{yy,CNN}^{M}'], 'fontsize', 16);
ylabel([char(949),'_{yy,SCA}^{M}'], 'fontsize', 16);
xlim([0 0.05])
ylim([0 0.05])
ax = gca;
ax.FontSize = 16;
...
subplot(1,3,3);
plot(eps_macro_prediction(:,3),eps_macro_validation(:,3),'g*');
hold on
...
T = text(max(get(gca, 'xlim'))*0.2, max(get(gca, 'ylim'))*0.8, str); 
set(T, 'fontsize', 16, 'verticalalignment', 'top', 'horizontalalignment', 'left');
legend([char(947),'_{xy,CNN}^{M} vs.',char(947),'_{xy, SCA}^{M}'],...
       [char(947),'_{xy,SCA}^{M} vs.',char(947),'_{xy, SCA}^{M}'], 'Location', 'southeast')
set(gca,'FontSize',fs);
xlabel([char(947),'_{xy,CNN}^{M}'], 'fontsize', 16);
ylabel([char(947),'_{xy,SCA}^{M}'], 'fontsize', 16);
xlim([0 0.05])
ylim([0 0.05])
ax = gca;
ax.FontSize = 16;
