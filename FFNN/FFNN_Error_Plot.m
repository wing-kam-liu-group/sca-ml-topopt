...
hist(l2, 60);
set(gca, 'FontSize', 16);
xlabel(['Difference in',char(8467),'^{2} norm'], 'fontsize', 16);
...
figure; 
subplot(1,3,1)
plot(sigma_sup_SCA_M(:,1), sigma_sup_FFNN_M(:,1), 'r*')
hold on
...
ylim([0 12])
set(gca, 'FontSize', 14);
xlabel([char(963),'_{xx}^{M}, MPa'], 'fontsize', fs);
ylabel([char(963),'_{xx}^{M}, MPa'], 'fontsize', fs);
ax = gca;
ax.FontSize = 16;

subplot(1,3,2)
plot(sigma_sup_SCA_M(:,2), sigma_sup_FFNN_M(:,2), 'blue*')
hold on
...
subplot(1,3,3)
plot(sigma_sup_SCA_M(:,3), sigma_sup_FFNN_M(:,3), 'g*')
hold on
...
set(gca, 'FontSize', fs);
xlabel([char(964),'_{xy}^{M}, MPa'], 'fontsize', fs);
ylabel([char(964),'_{xy}^{M}, MPa'], 'fontsize', fs);
ax = gca;
ax.FontSize = 16;
...
%% random seed j contorls which loading history to plot. In the paper, 16 is used.
% Users can also uncomment line 97 and comment line 98 to examine if the NN capable of capturing all loading cases
%j=ceil(rand*30)-1;
j = 16;
strain = zeros(1,6);
stress_sca = strain;
stress_nn = strain;
for i = 1:5
    strain(i+1) = epsilon_sup_M((i-1)*30+j, 1);
    stress_sca(i+1) = sigma_sup_SCA_M((i-1)*30+j, 1);
    stress_nn(i+1) = sigma_sup_FFNN_M((i-1)*30+j, 1);
end

plot(strain, stress_sca, 'r', 'linewidth', 2)
hold on
plot(strain, stress_nn, 'x', 'linewidth', 2, 'markersize', 10)
legend([char(963),'_{xx,SCA}^{M}'], [char(963),'_{xx,FFNN}^{M}'], 'Location', 'Best')
xlabel([char(949),'_{xx}^{M}'], 'fontsize', fs);
ylabel([char(963),'_{xx}^{M}, MPa'], 'fontsize', fs);
...
set(gca, 'FontSize', fs);
for i = 1:5
    strain(i+1) = epsilon_sup_M((i-1)*30+j, 2);
    stress_sca(i+1) = sigma_sup_SCA_M((i-1)*30+j, 2);
    stress_nn(i+1) = sigma_sup_FFNN_M((i-1)*30+j, 2);
end
plot(strain, stress_sca, 'r', 'linewidth', 2)
hold on
plot(strain, stress_nn, 'x', 'linewidth', 2, 'markersize', 10)
...
set(gca, 'FontSize', fs);
for i = 1:5
    strain(i+1) = epsilon_sup_M((i-1)*30+j, 3);
    stress_sca(i+1) = sigma_sup_SCA_M((i-1)*30+j, 3);
    stress_nn(i+1) = sigma_sup_FFNN_M((i-1)*30+j, 3);
end
plot(strain, stress_sca, 'r', 'linewidth', 2)
hold on
plot(strain, stress_nn, 'x', 'linewidth', 2, 'markersize', 10)
legend([char(964),'_{xy,SCA}^{M}'], [char(964),'_{xy,FFNN}^{M}'], 'Location', 'Best')
xlabel([char(947),'_{xy}^{M}'], 'fontsize', fs);
ylabel([char(964),'_{xy}^{M}, MPa'], 'fontsize', fs);
xlim([0 max(strain)*1.05])
ylim([0 max(stress_sca)*1.05])
set(gca, 'XTick', strain)
